...
export class L2MessagesServiceProxy {
    ...
    getAllMSG(): Observable<Root> {
        ...
        url_ = url_.replace(/[?&]$/, "");
        return this.http.get<Root>(url_).pipe(catchError(this.handleError));
    }
    ...
    getMessageHistory(): Observable<MessageDtoListResultDto> {
        let url_ = this.baseUrl + "/api/services/app/Message/GetMessageHistory";
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processGetMessages(response: HttpResponseBase): Observable<MessageDtoListResultDto> {
        const status = response.status;
        const rBlob =
            response instanceof HttpResponse ? response.body :
                (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        let _headers: any = {}; if (response.headers) { for (let key of response.headers.keys()) { _headers[key] = response.headers.get(key); } }
        if (status === 200) {
            return blobToText(rBlob).pipe(_observableMergeMap(_responseText => {
                let result200: any = null;
                let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
...
export class MessageDto implements IMessageDto {
    ...
    toJSON(data?: any) {
        ...
        data["dcreationTime"] = this.creationTime;
